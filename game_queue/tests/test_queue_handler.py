...
def test_queue_full():
    ...
    for player_id in range(0, 10):
        queue = game_queue.add_player(player_id, roles[player_id % 5], 0)

    assert len(queue) == 10

    # We queue our player 0 in channel 1, which he should be allowed to do
    queue_1 = game_queue.add_player(0, roles[0], 1)
    ...
    with pytest.raises(game_queue.PlayerInReadyCheck):
        game_queue.add_player(0, roles[0], 2)
    ...
    game_queue.add_player(10, roles[0], 0)
    # We also queue him in channel 1
    game_queue.add_player(10, roles[0], 1)
...
def test_queue_remove():
    ...
    queue = game_queue.add_player(0, roles[0], 0)
